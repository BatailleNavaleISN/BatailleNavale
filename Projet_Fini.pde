...
void Tir_Ordi ()
{
  ...
  if (compteCoule==5)
  {
    ...
    //On écrit que le joueur a perdu
    textFont(police, 80);
    text("PERDU!!!!!!", 260, 300);
  }
}
int [][] grandeGrille = new int [10][10];
void Tir_Joueur()
{
  int Tir_Joueur_x=mouseX;
  int Tir_Joueur_y=mouseY;

  while (grandeGrille [Tir_Joueur_x] [Tir_Joueur_y]==1);
  grandeGrille [Tir_Joueur_x] [Tir_Joueur_y]=1;
  for (int i=0; i<5; i=i+1)
  {
    TirReussi (flotteOrdi[i], Tir_Joueur_x, Tir_Joueur_y);
  }
  int compteCoule = 0;
  for (int i=0; i<5; i=i+1)
  {
    if (Coule(flotteOrdi[i])==true)
    {
      compteCoule = compteCoule+1;
    }
  }
...
{
  boolean BateauTouche =false;
  for (int i=0; i<5; i=i+1)
  {
    if (TirReussi (flotteOrdi[i], CaseX, CaseY)==true)
    {
      BateauTouche=true;
    }
  }
  if (BateauTouche==true)
  {
    stroke(222, 49, 99);
  } else
  {
  ...
  if (compteCoule==5)
  {
    ...
    fill(46, 0, 108);
    police=loadFont("GentiumBasic-48.vlw");
    //On écrit que le joueur a gagné
    textFont(police, 80);
    text("GAGNÉ!!!!!!", 260, 300);
  }
}
